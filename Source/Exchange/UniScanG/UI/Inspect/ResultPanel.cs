...
namespace UniScanG.UI.Inspect
{
    public partial class ResultPanel : UserControl, IMultiLanguageSupport
    {
        ...
        {
            if (inspectionResult.AlgorithmResultLDic.ContainsKey(FiducialFinderS.TypeName) == false)
                return;
            
            if (InvokeRequired)
            {
                BeginInvoke(new InspectDoneDelegate(FidInspectDone), inspectionResult);
                return;
            }

            AlgorithmResult algorithmResult = inspectionResult.AlgorithmResultLDic[FiducialFinderS.TypeName];
        ...
        public void SheetInspectDone(InspectionResult inspectionResult)
        {
            ...
            if (InvokeRequired)
            {
                BeginInvoke(new InspectDoneDelegate(SheetInspectDone), inspectionResult);
                return;
            }
            ...
            DataGridViewRow row = new DataGridViewRow();
            DataGridViewTextBoxCell indexCell = new DataGridViewTextBoxCell() { Value = inspectionResult.InspectionNo };
            DataGridViewTextBoxCell numCell = new DataGridViewTextBoxCell() { Value = sheetResult.SheetSubResultList.Count};
