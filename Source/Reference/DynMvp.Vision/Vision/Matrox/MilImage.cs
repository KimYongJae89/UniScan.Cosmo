...
namespace DynMvp.Vision.Matrox
{
    public abstract class MilImage : AlgoImage, MilObject
    {
        ...
        public override int Pitch
        {
            get
            {
                ...
                MIL_INT pitch = MIL.MbufInquire(image, MIL.M_PITCH_BYTE);
                return (int)pitch;
            }
        }

        public override int BitPerPixel => (int)MIL.MbufInquire(image, MIL.M_SIZE_BIT);
    ...
    public class MilBinalImage : MilImage
    {
        ...
        public MilBinalImage(int width, int height)
        {
            LibraryType = ImagingLibrary.MatroxMIL;
            ImageType = ImageType.Binary;
        ...
        public MilBinalImage(int width, int height, IntPtr dataPtr, int pitch = 0)
        {
            LibraryType = ImagingLibrary.MatroxMIL;
            ImageType = ImageType.Binary;
        ...
        public override void Alloc(int width, int height, IntPtr dataPtr, int pitch)
        {
            ...
            if (dataPtr == IntPtr.Zero)
            {
                ...
                image = MIL.MbufAlloc2d(MIL.M_DEFAULT_HOST, width, height, 1 + MIL.M_UNSIGNED, attribute, MIL.M_NULL);
            }
            else
            {
                image = MIL.MbufCreate2d(MIL.M_DEFAULT_HOST, width, height, 1 + MIL.M_UNSIGNED, attribute, MIL.M_HOST_ADDRESS + MIL.M_PITCH, MIL.M_DEFAULT, (ulong)dataPtr, MIL.M_NULL);
            }
    ...
    public class MilGpuImage : MilImage
    {
        ...
        public MilGpuImage(int width, int height)
        {
            LibraryType = ImagingLibrary.MatroxMIL;
            ImageType = ImageType.Gpu;
        ...
        public MilGpuImage(int width, int height, IntPtr dataPtr, int pitch = 0)
        {
            LibraryType = ImagingLibrary.MatroxMIL;
            ImageType = ImageType.Gpu;
