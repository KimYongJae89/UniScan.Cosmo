...
{
    public partial class OverlayPanel : UserControl, IResultPanel
    {
        ...
        ScatterLineSeries seriesOverlayLine;
        ScatterLineSeries seriesLastLine;
        ...
        public void Initialize(ResultPanelInfo resultPanelInfo)
        {
            ...
            seriesOverlayLine.Brush = new SolidBrush(Color.Pink);
            seriesOverlayLine.Thickness = 2;



            seriesLastLine = new ScatterLineSeries();

            seriesLastLine.XMemberPath = "X";
            seriesLastLine.YMemberPath = "Y";
            seriesLastLine.XAxis = xAxis;
            seriesLastLine.YAxis = yAxis;
            seriesLastLine.Brush = new SolidBrush(Color.Blue);
            seriesLastLine.Thickness = 2;
            ...
            profileChart.Series.Add(seriesLastLine);
        ...
        {
            int count = info.OverlayCount - 1;

            List<PointF> pointOverlayList = new List<PointF>();
            for (int i = scanDataList.Count() - 2; i >= 0; i--)
            {
                pointOverlayList.AddRange(scanDataList[i].SheetThicknessData.ValueList);
                count--;
            ...
            {
                List<PointF> pointLastList = scanDataList.Last().SheetThicknessData.ValueList;
                if (pointLastList.Count() > 0)
                    seriesLastLine.DataSource = pointLastList;
            }
        }
